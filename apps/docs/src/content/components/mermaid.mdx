...
`mermaid` is an **optional peer dependency** of `@cloudvoyant/vortex-react` and `@cloudvoyant/vortex-svelte` — the loading skeleton renders without it, and the library chunk is fetched only when a Mermaid component mounts, so it never lands in the SSR bundle. Install it in any app that renders diagrams:
...
<Code
  code={`import { Mermaid } from '@cloudvoyant/vortex-react';
...
A component map (`vortex-ui-mdx`) routes `code.language-mermaid` to the `Mermaid` component so the fence renders as a diagram; the same source is usable via the JSX `code` prop directly when you prefer an explicit component:
...
The `code` prop works identically from Svelte and plain JSX; the `vortex-ui-mdx` fence mapping for other MDX hosts is planned future work.
