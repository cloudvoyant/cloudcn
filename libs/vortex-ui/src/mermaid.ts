...
/** Mermaid `themeVariables` keys mapped to the vortex-ui shadcn CSS custom properties they
...
/** Converts a CSS color (vortex-ui tokens are `oklch()`, some with alpha) to the hex/rgba form
...
export async function renderMermaidSource(code: string): Promise<string> {
  ...
  const { svg } = await mermaid.render(`vortex-ui-mmd-${Date.now()}-${mermaidSeq++}`, code);
  return svg;
}
