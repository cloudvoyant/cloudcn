// apps/docs/src/components/HeroVortex.ts
// Animated DNA/vortex-ui hero background: two out-of-phase sine backbones with
...
export function initHeroVortex() {
  const canvas = document.querySelector<HTMLCanvasElement>('[data-hero-vortex-ui]');
  if (!canvas) return;
  ...
  const draw = (t: number) => {
    ...
    // Double vortex-ui — drawn horizontally in a rotated coordinate system so it
