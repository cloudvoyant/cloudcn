...
- [ ] Shared types, cva variants, `cn`, and theme tokens are in `@cloudvoyant/vertex-ui`, not a framework package.
...
- [ ] Composition: components use vertex-ui layout primitives (`Container`, `Stack`/`HStack`/`VStack`, `Row`/`Col`, `Center`) over raw flex divs, with no unnecessary nesting.
