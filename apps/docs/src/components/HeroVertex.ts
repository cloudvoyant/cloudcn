// apps/docs/src/components/HeroVertex.ts
// Animated DNA/vertex-ui hero background: two out-of-phase sine backbones with
...
export function initHeroVertex() {
  const canvas = document.querySelector<HTMLCanvasElement>('[data-hero-vertex-ui]');
  if (!canvas) return;
  ...
  const draw = (t: number) => {
    ...
    // Double vertex-ui — drawn horizontally in a rotated coordinate system so it
