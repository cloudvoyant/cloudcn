// libs/helix/src/themes.ts
// Theme registry for helix's shadcn-compatible theme presets.
// Ported from diffbook — each name maps to a `.theme-{name}` / `.theme-{name}.dark`
// block in `src/themes/{name}.css` (all imported by `@cloudvoyant/helix/themes.css`).
