// libs/wicn-core/src/nav-menu.ts
...
//
// Density (root) and variant (item) are resolved DIRECTLY via cva — the
// framework wrappers pass the values through context, so no data-attribute /
// group-data selectors are needed. Top-level links use `navMenuTriggerStyle`
// (so they match the triggers); dropdown content links use `navMenuLinkBase`.
import { cva } from 'class-variance-authority';

export const navMenuRootBase = 'group/nav-menu relative flex max-w-max flex-1 items-center justify-center';

export const navMenuListBase = 'group flex flex-1 list-none items-center justify-center gap-1';

export const navMenuItemBase = 'relative';

export type NavMenuDensity = 'relaxed' | 'compact';

export type NavMenuVariant = 'default' | 'link';

export const navMenuTriggerStyle = cva(
  'group inline-flex h-9 w-max items-center justify-center rounded-md bg-background px-4 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 data-[state=open]:bg-accent/50 data-[state=open]:text-accent-foreground data-[state=open]:hover:bg-accent data-[state=open]:focus:bg-accent',
  {
    variants: {
      density: {
        relaxed: '',
...
export const navMenuContentBase =
...
export const navMenuIndicatorBase =
