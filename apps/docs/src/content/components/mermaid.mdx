...
A component map (`helix-mdx`) routes `code.language-mermaid` to the `Mermaid` component so the fence renders as a diagram; the same source is usable via the JSX `code` prop directly when you prefer an explicit component:
...
The `code` prop works identically from Svelte and plain JSX; the `helix-mdx` fence mapping for other MDX hosts is planned future work.
