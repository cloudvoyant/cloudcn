...
4. Only now apply vertex-ui adaptations — layout primitives, nesting reduction, Tailwind/token swaps, theme extensions — each in its own commit so a bad change can be reverted.
...
Use `@cloudvoyant/vertex-ui`'s `cn` (clsx + tailwind-merge) to combine variant classes with user `className`/`class`:
...
- The theme is the shadcn CSS-variable model in `@cloudvoyant/vertex-ui/src/theme.css`: `@theme inline` maps `--color-*` to `--background`/`--primary`/etc., with `:root` and `.dark` blocks. Do not diverge from shadcn's implementation.
...
- Never require a consumer to add CSS themselves; the extension ships in `@cloudvoyant/vertex-ui`'s `theme.css`.
