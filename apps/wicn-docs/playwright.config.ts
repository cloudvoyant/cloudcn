// apps/wicn-docs/playwright.config.ts
...
export default defineConfig({
  ...
  retries: 2,
  use: {
    baseURL: process.env.PLAYWRIGHT_BASE_URL ?? 'http://localhost:4321/wicn/',
  },
  webServer: {
    ...
    command:
      ...
      'for i in $(seq 1 120); do curl -fsS -o /dev/null http://localhost:4321/wicn/ && break; sleep 1; done; ' +
      "trap 'pnpm run preview stop >/dev/null 2>&1 || true' EXIT; " +
      'while true; do sleep 5; done',
    url: 'http://localhost:4321/wicn/',
