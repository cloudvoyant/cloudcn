...
4. Only now apply helix adaptations — layout primitives, nesting reduction, Tailwind/token swaps, theme extensions — each in its own commit so a bad change can be reverted.
...
Use `@cloudvoyant/helix`'s `cn` (clsx + tailwind-merge) to combine variant classes with user `className`/`class`:
...
- The theme is the shadcn CSS-variable model in `@cloudvoyant/helix/src/theme.css`: `@theme inline` maps `--color-*` to `--background`/`--primary`/etc., with `:root` and `.dark` blocks. Do not diverge from shadcn's implementation.
...
- Never require a consumer to add CSS themselves; the extension ships in `@cloudvoyant/helix`'s `theme.css`.
