// libs/vertex-ui/src/themes.ts
// Theme registry for vertex-ui's shadcn-compatible theme presets.
// Ported from diffbook — each name maps to a `.theme-{name}` / `.theme-{name}.dark`
// block in `src/themes/{name}.css` (all imported by `@cloudvoyant/vertex-ui/themes.css`).
...
export const THEMES: ThemeMeta[] = [
  ...
  { name: 'vertex-ui', label: 'Vertex UI' },
];
