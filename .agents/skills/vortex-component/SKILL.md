---
description: 'Create, update, or review vortex-ui components. Triggers on: "create component", "new component", "add a {X} component", "update component", "review component", "wrap {X} from ark ui", "/vortex-component".'
name: vortex-component
license: MIT
...
- Shared interfaces, cva variants, `cn`, and the theme live in `@cloudvoyant/vortex-ui` — never in a framework package (see `references/package-layout.md`)
...
- Use vortex-ui layout primitives (`Container`, `Stack`/`HStack`/`VStack`, `Row`/`Col`, `Center`) as heavily as possible and reduce unnecessary nesting — a deliberate difference from the shadcn approach (see `references/guidelines.md`)
