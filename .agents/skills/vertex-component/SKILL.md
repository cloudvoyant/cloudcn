---
description: 'Create, update, or review vertex-ui components. Triggers on: "create component", "new component", "add a {X} component", "update component", "review component", "wrap {X} from ark ui", "/vertex-component".'
...
- Shared interfaces, cva variants, `cn`, and the theme live in `@cloudvoyant/vertex-ui` — never in a framework package (see `references/package-layout.md`)
...
- Use vertex-ui layout primitives (`Container`, `Stack`/`HStack`/`VStack`, `Row`/`Col`, `Center`) as heavily as possible and reduce unnecessary nesting — a deliberate difference from the shadcn approach (see `references/guidelines.md`)
