// libs/vortex-ui/src/themes.ts
// Theme registry for vortex-ui's shadcn-compatible theme presets.
// Ported from diffbook — each name maps to a `.theme-{name}` / `.theme-{name}.dark`
// block in `src/themes/{name}.css` (all imported by `@cloudvoyant/vortex-ui/themes.css`).
...
export const THEMES: ThemeMeta[] = [
  ...
  { name: 'vortex-ui', label: 'Vortex UI' },
];
