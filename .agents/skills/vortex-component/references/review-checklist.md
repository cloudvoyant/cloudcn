...
- [ ] Shared types, cva variants, `cn`, and theme tokens are in `@cloudvoyant/vortex-ui`, not a framework package.
...
- [ ] Composition: components use vortex-ui layout primitives (`Container`, `Stack`/`HStack`/`VStack`, `Row`/`Col`, `Center`) over raw flex divs, with no unnecessary nesting.
