# vortex-ui architecture

## Monorepo layout

vortex-ui is a pnpm workspace monorepo:

- `libs/vortex-ui` — shared interfaces, cva variants (`buttonVariants`, `toggleButtonVariants`, `badgeVariants`), `cn` utility, and the shadcn-style theme (CSS variables, light + dark). Publishes to npm.
- `libs/vortex-react` — React UI components built on Ark UI (Button, ToggleButton, Badge). Publishes to npm.
- `libs/vortex-svelte` — Svelte UI components built on Ark UI (Button, ToggleButton, Badge), packaged with `@sveltejs/package`. Publishes to npm.
...
- The theme lives in `@cloudvoyant/vortex-ui/src/theme.css` and matches shadcn/ui's implementation exactly at this stage (CSS variables under `:root`/`.dark`, `@theme inline`, `@custom-variant dark`).
