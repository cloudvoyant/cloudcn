...
**The first pass is a working copy, never written from scratch** — pull a real, working implementation from Ark/Chakra (MCP snippet) or shadcn, then adapt it to wicn standards.
...
Ark UI is the library wicn wraps directly. Check it first.
...
- Adapt the snippet to wicn standards — `cn`, Tailwind, our theme — without sacrificing its accessibility.
...
<!-- Closely based on: @ark-ui/svelte/tooltip (Ark UI), mirrored from wicn-react -->
```
