<!-- libs/vertex-svelte/src/YoutubeTimestamps.svelte -->
<!-- Composed timestamps list, auto-inferred from web metadata or overridden manually via
     YoutubeTimestampAt children. Mirrored from @cloudvoyant/vertex-react. -->
<script lang="ts">
  ...
  const ctx = getContext<{ url: string; seek: (t: number) => void }>('vertex-ui-youtube');
